...
class ConditionalDiffusionModel(pl.LightningModule):
    """Pytorch lignting module that wraps a diffusion model composed of a
    noise prediction model and a noise scheduler coming from the diffusers
    library: https://huggingface.co/docs/diffusers/index. Particularly, the
    noise prediction model is conditioned on a prior such as a mask or external
    spinal line, in addition to the input noise."""

    def __init__(self, model, scheduler, loss, metrics, **kwargs):
        """Initializes the model.
    ...
    def forward(self, batch: list, batch_idx: int) -> torch.Tensor:
        """Computes a training/validation/test step.
        ...
        noisy_x = self.scheduler.add_noise(x, noise, timesteps)
        noisy_input = torch.cat([noisy_x, prior], dim=1)
        noise_pred = self.model(
            noisy_input.type(torch.float32), timesteps, return_dict=False
        )[0]
    ...
    def training_step(self, batch: list, batch_idx: int) -> torch.Tensor:
        ...
        Returns:
            loss (torch.Tensor): The loss value.
        """
        loss, metrics = self.forward(batch, batch_idx)
    ...
    def predict_step(
        ...
        """Performs a conditioned inference step.
        ...
        for t in tqdm(timesteps):
            with torch.no_grad():
                ...
                noisy_input = torch.cat([x, prior], dim=1)
                noisy_residual = self.model(noisy_input, t).sample
